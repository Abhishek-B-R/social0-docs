...
Both objects are merged into the post's stored metadata before publish. Prefer **`metadata`** when you control the full shape; use **`platform_options`** for a flatter, per-platform API that Social0 compiles into metadata (see [Posts reference](/docs/api/reference/list-posts)).
...
See [Jobs reference](/docs/api/reference/get-job) and [Publish guide](/docs/api/guides/publish).
