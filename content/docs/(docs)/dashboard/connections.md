...
**For API and MCP:** connect accounts in the dashboard first, then use `GET /v1/accounts` or MCP `list_accounts`. Twitter/X and Bluesky must be connected in the dashboard. See [Connect accounts (API)](/docs/api/guides/connect-accounts) and [MCP](/docs/mcp).
