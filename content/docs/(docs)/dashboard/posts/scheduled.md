...
**Q: What if a scheduled post fails to send to Twitter/X?**  
A: Check the post detail for the error. Common causes: X rate limits, token reconnect needed, or Free’s lifetime post quota. Fix the account or upgrade if needed, then retry from the Posts list.
