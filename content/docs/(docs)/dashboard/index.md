---
title: "Dashboard"
description: Your home base in Social0-where you create posts, manage connections, and see everything in one place.
---

## Overview

The Dashboard is where you spend most of your time in Social0. When you sign in, you land here. On **Free** you can connect up to **3** accounts and publish or schedule up to **10** lifetime posts (drafts do not count). Paid plans raise account limits and remove the free-post cap. From here you can open the Composer to create a post, go to Connections to add accounts, open Posts to see drafts and scheduled and published posts, change Settings, and open Billing. Everything is one or two clicks away.

You can also use Social0 programmatically via the [REST API](/docs/api/quickstart) or [MCP Server](/docs/mcp) for AI assistants-create API keys in **Developer** settings and publish from your backend or Claude/Cursor.

## How to get to the Dashboard

1. Go to [social0.app](https://social0.app) and sign in (Google or email).
2. After sign-in you’re taken straight to the Dashboard (usually the Composer screen).

If you’re already signed in, click the Dashboard link in the header or go to [social0.app](https://social0.app).

## What you’ll see

At the top of the sidebar you’ll see a **Create post** button-the quickest way to start a new post. Below that, the sidebar is grouped into sections:

**CREATE**

- **Composer** - The main place to write a post and click Continue to pick platforms and publish or schedule.
- **Manual setup** - Choose the post type yourself (text, image, video, thread, or collection) and fill in the form.
- **Bulk tools** - Upload and schedule many images or videos at once (Growth and Pro plans).

**POSTS**

- **All** - List of all your posts (drafts, scheduled, published). Open any post to view, edit, or retry.
- **Posted** - Posts that have already been sent to your connected accounts.
- **Scheduled** - Posts set to go out at a specific time or in a queue slot.
- **Drafts** - Posts you’ve saved but not yet published or scheduled.
- **Calendar** - View your scheduled and published posts on a calendar.

**WORKSPACE**

- **Workspace switcher** - Switch between **Main (personal)** and team/solo workspaces.
- **Connections** - Connected accounts for the **active** workspace (Main or team). Owner/Admin manage; Members can view in a team.
- **Teams** - Collaborative teams (Pro): invite, roles, settings. See [Teams & Workspaces](/docs/dashboard/teams).
- **Workspaces** - Board to create solo/team workspaces and move connections. See [Workspaces board](/docs/dashboard/workspaces).

**CONFIGURATION**

- **Settings** - Profile, timezone, date format, queue slots, and other preferences (personal).
- **Developer** - Create API keys and manage webhooks for programmatic access. See [Developer settings](/docs/dashboard/api-keys), the [REST API](/docs/api/quickstart), and [MCP Server](/docs/mcp) for AI assistants.
- **Billing** - Your personal plan, usage, and upgrade or cancel (team Pro is the owner’s subscription).

**SUPPORT**

- **Feedback** - Send feedback or feature requests to the Social0 team.

**RESOURCES**

- **Latest updates** - See what’s new.
- **Docs** - Link to this documentation.

## Tips

- Save the Dashboard page in your browser's bookmarks/favorites for quick access later.
- Use the **Composer** for the fastest path from idea to publish; use **Manual setup** when you want to pick the post type yourself.
